...
- [ ] **Level-change notifications:** When a concept changes trust level (untested → inferred, inferred → verified, etc.), an inline annotation appears in conversation:
  - Format: `─── concept-name → level (reason) ───`
  - `--font-data`, `--text-xs`, `--stone`, centered
  - "[see reasoning]" link opens `explainDecision` in margin panel
- [ ] **Challenge flow:** Learner can click any concept on the map/margin and:
...
- [ ] **Quiet mode (optional toggle):** When enabled, implicit signal writes are buffered as "pending signals"
...
- [ ] Level-change toast: translateY 8→0, opacity 0→1, 300ms, auto-dismiss 5s unless interacted with
- [ ] All trust state changes traceable — nothing written to core without the learner being able to see it afterward
